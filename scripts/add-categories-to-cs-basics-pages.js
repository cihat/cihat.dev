...
const postsDir = path.join(__dirname, '../app/(post)/2024/cs-basics-series');

// CS Basics Series sub-pages should have "Learning" category
const csBasicsCategory = 'Learning';

// Recursively find all page.mdx files in cs-basics-series directory
...
mdxFiles.forEach((filePath) => {
  try {
    ...
    // Skip the main cs-basics-series page (it's already handled)
...
<div className="text-sm text-gray-500 dark:text-gray-500 mb-10 pb-6 border-b border-gray-200 dark:border-gray-700">
  <span className="font-semibold">${date}</span> • <span className="font-semibold">${csBasicsCategory}</span> • <span>${minuteToRead} mins read</span>
</div>
...
`;
      ...
      if (match) {
        ...
        if (oldCategory !== csBasicsCategory) {
          content = content.replace(categoryRegex, `$1${csBasicsCategory}$4`);
          fs.writeFileSync(filePath, content, 'utf8');
          console.log(`✓ Updated ${relativePath}: ${oldCategory} → ${csBasicsCategory}`);
        }
      }
    }
  } catch (error) {
