...
function Card(props: CardType) {
  ...
  return (
    <div className="flex flex-col p-4 border radious rounded-md h-max" key={props.key}>
      <div className="flex">
      ...
      <div className="flex items-center mt-2">
        ...
        <A href={`${link}/stargazers`} target="_blank" className="flex items-center mr-2  border-none hover:text-[#2f81f7] dark:hover:text-[#2f81f7]">
          <AiOutlineStar className="mr-0.5" />
          {stars}
        </A>
        <A href={`${link}/forks`} target="_blank" className="flex items-center border-none hover:text-[#2f81f7] dark:hover:text-[#2f81f7]">
          <AiOutlineBranches className="mr-0.5" />
          {forks}
        </A>
      </div>
      <Image src={image} width={500} height={250} alt={repo} loading="lazy" />
    </div>
  )
}
